...
| `steps.clair3.model` | Public pretrained models linked from [Clair3](https://github.com/HKU-BAL/Clair3#pre-trained-models). Download/extract the model matching your ONT chemistry and basecalling model; this is a directory. The locked container's Clair3 version must support it. There is no one model determined solely by genome build. |
